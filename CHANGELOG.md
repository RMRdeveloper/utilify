...
- **isEmpty Test Fix**: Corrected test expectations for Map and Set with entries in `is-empty.test.ts`
  - Fixed assertions expecting `true` for non-empty Map and Set instances to expect `false`
  - Ensures tests accurately reflect the `isEmpty` function behavior checking `size` property
